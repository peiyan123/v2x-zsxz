...
class CameraController extends BaseController {
  ...
  async get() {
    ...
    const result = await this.service.camera.getByNameAndRtsp({ name, rtsp }, orderInfo, pageParams);
    this.success(result);
  }
  ...
  async create() {
    ...
    const valid = await ctx.validate('camera', requestData, 'create');
    if (!valid) return;
    // const cameraName = positionName[String(requestData.position)];
    const cameraName = await this.service.group.getGroupNameById(requestData.groupId);
    if (requestData.rtsp) {
    ...
    const result = await this.service.camera.create(requestData);
    if (result.returnStatus === 'SUCCEED') {
      // 新建摄像头时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail({ deviceSn: requestData.sn, id: result.data.id, operatorType: 'VIDEO', uri: requestData.rtsp, groupName: cameraName });
      this.success();
    } else {
  ...
  async update() {
    const { ctx } = this;
    const requestData = ctx.request.body;
    const valid = await ctx.validate('camera', requestData, 'update');
    if (!valid) return;
    // const cameraName = positionName[String(requestData.position)];
    const cameraName = await this.service.group.getGroupNameById(requestData.groupId);
    if (requestData.rtsp) {
    ...
    // 获取状态值
    requestData.status = this.getStatus(requestData);
    const result = await this.service.camera.update(requestData);
    if (result.returnStatus === 'SUCCEED') {
      // 更新摄像头时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail({ deviceSn: requestData.sn, id: requestData.id, operatorType: 'VIDEO', uri: requestData.rtsp, groupName: cameraName });
      this.success();
    } else {
      this.failed(result.errorMessage);
    }
  ...
  async delete() {
    ...
    const valid = await ctx.validate('camera', ctx.query, 'delete');
    if (!valid) return;
    ...
    const result = await this.service.camera.delete(cameraId);
    // 删除摄像头时需调用EdgeService的删除通道算法配置接口
    // this.service.edge.deleteDetail([{ operatorType: 'VIDEO', ids: [ cameraId ] }]);
    let data = {
      analyIds: [
        { 
          operatorType: 'VIDEO', 
  ...
  async deleteCameraConfig(dirName) {
    const dir = this.app.config.algorithmicDir + 'chan1/cfg_single/' + dirName;
