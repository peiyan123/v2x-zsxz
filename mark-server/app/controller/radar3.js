...
class RadarController extends BaseController {
  async get() {
    const { ctx } = this;
    const result = await this.service.radar3.getAll();
    this.success(result);
  }
  async create() {
    const { ctx } = this;
    const requestData = ctx.request.body;
    const valid = await ctx.validate('radar', requestData, 'create');
    if (!valid) return;
    const result = await this.service.radar3.create(requestData);
    if (result.returnStatus === 'SUCCEED') {
      const groupName = await this.service.group3.getGroupNameById(requestData.groupId);
      // 新建雷达时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail3({ radarType: requestData.model, operatorType: 'LIDAR', radarIp: requestData.ip, deviceSn: requestData.sn, id: result.data.id, groupName, ...pick(requestData, [ 'port', 'protocol', 'mbcIp', 'mbcPort', 'mbcNetworkCard' ]) });
      this.success();
    } else {
  ...
  async update() {
    ...
    const valid = await ctx.validate('radar', requestData, 'update');
    if (!valid) return;
    const result = await this.service.radar3.update(requestData);
    if (result.returnStatus === 'SUCCEED') {
      const groupName = await this.service.group3.getGroupNameById(requestData.groupId);
      // 更新雷达时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail3({ radarType: requestData.model, operatorType: 'LIDAR', radarIp: requestData.ip, deviceSn: requestData.sn, groupName, ...pick(requestData, [ 'id', 'port', 'protocol', 'mbcIp', 'mbcPort', 'mbcNetworkCard' ]) });
      this.success();
    } else {
      this.failed(result.errorMessage);
    }
  ...
  async delete() {
    ...
    const valid = await ctx.validate('radar', ctx.query, 'delete');
    if (!valid) return;
    const isExist = await this.app.model.Radar3.findOne({ where: { id: radarId } });
    if (!isExist) return this.failed('ID不存在');
    // const dir = positionName[String(isExist.position)];
    const dir = await this.service.group3.getGroupNameById(isExist.groupId);
    const result = await this.service.radar3.delete(radarId);
  ...
  async writeDeviceData() {
    ...
    const filePath = 'app/public/files/default-empty3.json';
    const emptyArr = [];
    const allDeviceData = await this.service.radar3.getAll();
  ...
  async reportData() {
    ...
    for (let i = 0; i < allDeviceData.length; i++) {
      const groupName = await this.service.group3.getGroupNameById(allDeviceData[i].groupId);
  ...
  async writeRreportData(data) {
    const isDirExsit = await fsExtra.pathExists(this.app.config.algorithmicDir + 'chan3/cfg_single/' + data.positionName);
    if (!isDirExsit) return false;
    const fileName = this.app.config.algorithmicDir + 'chan3/cfg_single/' + data.positionName + '/sensor.json';
  ...
  async deleteRadarConfig(dir) {
    const fileName = this.app.config.algorithmicDir + 'chan3/cfg_single/' + dir + '/sensor.json';
    const isFileExsit = await fsExtra.pathExists(fileName);
    if (isFileExsit) {
      const fileData = await fsExtra.readJson(fileName);
