...
class GroupController extends BaseController {
   ...
   */
  async getAll() {
    const result = await this.service.group3.getAll();
    this.success(result);
  }
  /**
   * 获取组的基本信息和设备信息
  ...
  async getGroupInfoById() {
    ...
    const result = await this.service.group3.getGroupInfoById(id);
  ...
  async create() {
    ...
    if (!valid) return;
    const result = await this.service.group3.create(requestData);
    if (result.returnStatus === 'SUCCEED') {
      this.success(result.data);
    } else {
      this.failed(result.errorMessage);
  ...
  async update() {
    ...
    if (!valid) return;
    const result = await this.service.group3.update(requestData);
    if (result.returnStatus === 'SUCCEED') {
      this.success();
    } else {
      this.failed(result.errorMessage);
  ...
  async delete() {
    ...
    if (!valid) return;
    const result = await this.service.group3.delete(id);
    if (result.returnStatus === 'SUCCEED') {
      ...
      if (result && result.data && result.data.name) {
        ...
        deleteFolderRecursive(`${this.app.config.algorithmicDir}chan3/cfg_single/${result.data.name}`);
      }
      this.success();
    } else {
      this.failed(result.errorMessage);
  ...
  async pullDataFromExtension() {
    const { ctx } = this;
    const id = ctx.query.id;
    const { data } = await this.service.group3.getGroupInfoById(id);
    if (data.extensionIp) {
      try {
        // 发送请求，获取数据
        ...
        if (result.returnStatus === 'SUCCEED') {
          ...
          if (data) {
            // 更新group的dataPort字段
            await this.service.group3.update({ id, dataPort: data.dataPort });
            if (data.camera) {
              ...
              // 更新camera
              const { data: cameraId } = await this.service.camera3.upsertCamera(data.camera);
              const mark = data.mark;
              if (mark && mark.length > 0 && cameraId) {
                // const cameraId = mark[0].camera_id;
                const imageInfo = mark[0].image_info;
                // 更新标注信息
                await this.service.mark3.create(cameraId, mark, imageInfo);
              }
            }
            if (data.radar) {
              // 修改group id，避免拉取的和当前id不一致
              ...
              // 更新radar
              await this.service.radar3.upsertRadar(data.radar);
            }
          }
          this.success();
        } else {
  ...
  async getAllInfoByGroupName() {
    ...
    const result = await this.service.group3.getGroupInfoByName(name);
    if (result.returnStatus === 'SUCCEED') {
      const { data } = result;
      if (!data) {
        ...
        if (data.camera) {
          ...
          // 获取标注数据
          markData = await this.service.mark3.getMarkData(cameraId);
  ...
  async uploadConfigFiles() {
    ...
    await this.service.group3.deleteAllGroups();
    await fsExtra.emptyDir(this.app.config.algorithmicDir + 'chan3/');
    ...
    while ((stream = await parts()) != null) {
      if (stream.filename) {
        ...
        await this.service.group3.saveConfigFiles(stream, cameraIds);
    ...
    // 更新相机标定页面的图片
    await this.service.group3.updateCameraImage(cameraIds);
    this.success('参数上传成功');
  }
}
