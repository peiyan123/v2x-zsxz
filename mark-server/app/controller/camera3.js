...
class CameraController extends BaseController {
  ...
  async get() {
    ...
    const result = await this.service.camera3.getByNameAndRtsp({ name, rtsp }, orderInfo, pageParams);
    this.success(result);
  }
  ...
  async create() {
    ...
    const cameraName = await this.service.group3.getGroupNameById(requestData.groupId);
    if (requestData.rtsp) {
    ...
    const result = await this.service.camera3.create(requestData);
    if (result.returnStatus === 'SUCCEED') {
      // 新建摄像头时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail3({ deviceSn: requestData.sn, id: result.data.id, operatorType: 'VIDEO', uri: requestData.rtsp, groupName: cameraName });
      this.success();
    } else {
  ...
  async update() {
    ...
    // 获取状态值
    requestData.status = this.getStatus(requestData);
    const result = await this.service.camera3.update(requestData);
    if (result.returnStatus === 'SUCCEED') {
      // 更新摄像头时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail3({ deviceSn: requestData.sn, id: requestData.id, operatorType: 'VIDEO', uri: requestData.rtsp, groupName: cameraName });
      this.success();
    } else {
      this.failed(result.errorMessage);
    }
  ...
  async delete() {
    ...
    const result = await this.service.camera3.delete(cameraId);
    // 删除摄像头时需调用EdgeService的删除通道算法配置接口
    // this.service.edge.deleteDetail3([{ operatorType: 'VIDEO', ids: [ cameraId ] }]);
    let data = {
      analyIds: [
        { 
          operatorType: 'VIDEO', 
  ...
  async deleteCameraConfig(dirName) {
    const dir = this.app.config.algorithmicDir + 'chan3/cfg_single/' + dirName;
