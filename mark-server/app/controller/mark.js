...
class MarkController extends BaseMarkController {
  async handleMarkData() {
    ...
    if (!valid) return;
    const isExist = await this.app.model.Camera.findOne({ where: { id: cameraId } });
    if (!isExist) return this.failed('不存在此设备');
    const result = await this.service.mark.handleMarkData(requestData);
    this.success(result);
  }
  async getMarkData() {
    const { ctx } = this;
    ...
    const isExist = await this.app.model.Camera.findOne({ where: { id: cameraId } });
    if (!isExist) return this.failed('不存在此设备');
    const result = await this.service.mark.getMarkData(cameraId);
    this.success(result);
  }
  ...
  async download() {
    ...
    const position = await this.service.group.getGroupNameById(camera.groupId);
    ...
    try {
      const sourceDir = `${this.app.config.algorithmicDir}chan1/cfg_single/${position}`;
  ...
  async getImage() {
    ...
    if (!valid) return;
    const camera = await this.app.model.Camera.findOne({ where: { id } });
    ...
    const groupName = await this.service.group.getGroupNameById(camera.groupId);
  ...
  async reportData() {
    const { ctx } = this;
    let index = 0;
    const { resultGroup, cameraData } = await ctx.service.mark.getReportData();
    const handleArr = [];
    ...
    for (const key in cameraData) {
      ...
      const groupInfo = await this.service.group.getGroupBasicInfoById(cameraItemData.groupId);
      if (!groupInfo) {
        continue;
      }
      ...
      this.writeSensorData(cameraItemData, groupInfo, index, '1');
      const fuseArrTemp = this.handleData(markData, cameraItemData, '1');
  ...
  async writeFuseData(entryAreaData) {
    ...
    const filePath = `${this.app.config.algorithmicDir}chan1/cfg_multiple/fuse_multiple.json`;
    await fsExtra.ensureDir(this.app.config.algorithmicDir + 'chan1/cfg_multiple/');
  ...
  async writeDeviceData() {
    ...
    const emptyArr = [];
    const allDeviceData = await this.service.camera.getAll();
  ...
  async checkCameraDataImg() {
    ...
    const groupIdResult = await this.service.camera.getGroupId(requestData.cameraId);
    if (!groupIdResult) {
      return this.failed('未获取到摄像头所在组');
    }
    const groupName = await this.service.group.getGroupNameById(groupIdResult.groupId);
  ...
  async writeSpeedThreshold(data) {
    ...
    const fileDir = `${this.app.config.algorithmicDir}chan1/cfg_single/${data.name}/event_param`;
  ...
  async writeOtherEventParams(groupName) {
    ...
    const fileDir = `${this.app.config.algorithmicDir}chan1/cfg_single/${groupName}/event_param`;
    await fsExtra.ensureDir(fileDir);
  ...
  async downloadExtrinsicBin() {
    ...
    // 下载文件路径
    const file = this.app.config.algorithmicDir + 'chan1/cfg_single/' + group + commonDir + '/' + extrinsicBin;
