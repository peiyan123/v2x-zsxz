...
class MarkController extends BaseMarkController {
  async handleMarkData() {
    ...
    const valid = await this.ctx.validate('mark2', ctx.request.body, 'saveMark');
    if (!valid) return;
    const isExist = await this.app.model.Camera2.findOne({ where: { id: cameraId } });
    if (!isExist) return this.failed('不存在此设备');
    const result = await this.service.mark2.handleMarkData(requestData);
    this.success(result);
  }
  async getMarkData() {
    const { ctx } = this;
    ...
    const isExist = await this.app.model.Camera2.findOne({ where: { id: cameraId } });
    if (!isExist) return this.failed('不存在此设备');
    const result = await this.service.mark2.getMarkData(cameraId);
    this.success(result);
  }
  ...
  async download() {
    ...
    const camera = await this.app.model.Camera2.findOne({ where: { id: cameraId } });
    if (!camera) {
      return this.failed('不存在此设备');
    }
    const position = await this.service.group2.getGroupNameById(camera.groupId);
    ...
    try {
      const sourceDir = `${this.app.config.algorithmicDir}chan2/cfg_single/${position}`;
  ...
  async getImage() {
    ...
    const valid = await this.ctx.validate('mark2', this.ctx.query, 'image');
    if (!valid) return;
    const camera = await this.app.model.Camera2.findOne({ where: { id } });
    ...
    const groupName = await this.service.group2.getGroupNameById(camera.groupId);
  ...
  async reportData() {
    const { ctx } = this;
    let index = 0;
    const { resultGroup, cameraData } = await ctx.service.mark2.getReportData();
    const handleArr = [];
    ...
    for (const key in cameraData) {
      ...
      const groupInfo = await this.service.group2.getGroupBasicInfoById(cameraItemData.groupId);
      if (!groupInfo) {
        continue;
      }
      ...
      this.writeSensorData(cameraItemData, groupInfo, index, '2');
      const fuseArrTemp = this.handleData(markData, cameraItemData, '2');
  ...
  async writeFuseData(entryAreaData) {
    ...
    const filePath = `${this.app.config.algorithmicDir}chan2/cfg_multiple/fuse_multiple.json`;
    await fsExtra.ensureDir(this.app.config.algorithmicDir + 'chan2/cfg_multiple/');
  ...
  async writeDeviceData() {
    ...
    const emptyArr = [];
    const allDeviceData = await this.service.camera2.getAll();
  ...
  async checkCameraDataImg() {
    ...
    const groupIdResult = await this.service.camera2.getGroupId(requestData.cameraId);
    if (!groupIdResult) {
      return this.failed('未获取到摄像头所在组');
    }
    const groupName = await this.service.group2.getGroupNameById(groupIdResult.groupId);
  ...
  async writeSpeedThreshold(data) {
    ...
    const fileDir = `${this.app.config.algorithmicDir}chan2/cfg_single/${data.name}/event_param`;
  ...
  async writeOtherEventParams(groupName) {
    ...
    const fileDir = `${this.app.config.algorithmicDir}chan2/cfg_single/${groupName}/event_param`;
    await fsExtra.ensureDir(fileDir);
  ...
  async downloadExtrinsicBin() {
    ...
    // 下载文件路径
    const file = this.app.config.algorithmicDir + 'chan2/cfg_single/' + group + commonDir + '/' + extrinsicBin;
