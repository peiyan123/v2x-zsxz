...
class MecController extends BaseController {
  ...
  async saveConfig() {
    ...
    // if (!valid) return;
    await this.service.mec3.saveMecConfig(requestData);
    // 写入配置文件，mqtt信息，经纬度
    // const filePath = this.deviceConfigPath;
    const filePathLocal = 'app/public/files/default-empty3.json';
    ...
    if (requestData.deviceId !== undefined) {
      // this.service.edge.saveDeviceDetail3(requestData);
    }
    this.success();
  }
  /**
  ...
  async saveMqttConfig(requestData) {
    ...
    const result = {
      ...
      tls: {
        enable: false,
        capath: '/opt/soft/v2x/chan3/cfg_multiple/publish_task/tls',
        cafile: '/opt/soft/v2x/chan3/cfg_multiple/publish_task/tls/ca.pem',
        certfile: '/opt/soft/v2x/chan3/cfg_multiple/publish_task/tls/app.pem',
        keyfile: '/opt/soft/v2x/chan3/cfg_multiple/publish_task/tls/app.key',
      },
      qos: 1,
      keeplive: 10,
      timeout: 100,
    ...
    const dirPath = `${this.app.config.algorithmicDir}chan3/cfg_multiple/publish_task`;
    await fsExtra.ensureDir(dirPath);
    await fsExtra.writeJson(`${dirPath}/mqttalgodata_task3.json`, result, { spaces: '\t' });
  }
  /**
  ...
  async saveDeviceInfoConfig(requestData) {
    ...
    await fsExtra.ensureDir(dirPath);
    await fsExtra.writeJson(`${dirPath}device_info3.json`, result, { spaces: '\t' });
  }

  async getConfig() {
    const result = await this.service.mec3.getMecConfig(1) || {};
  ...
  async getRoadPicture() {
    ...
    const filePath = `${roadInfoPath}/chan3/topview_bg3.jpg`;
  ...
  async getRoadConfig() {
    const { roadInfoPath } = this.app.config;
    const filePath = `${roadInfoPath}/chan3/topview_bg.jpg.cfg.json`;
    if (fsExtra.existsSync(filePath)) {
  ...
  async testConnect() {
    ...
    if (!requestData.username || !requestData.password) {
      try {
        ca = fs.readFileSync('/opt/soft/v2x/configs/chan3/cfg_multiple/publish_task/tls/ca.pem');
        cert = fs.readFileSync('/opt/soft/v2x/configs/chan3/cfg_multiple/publish_task/tls/app.pem');
        key = fs.readFileSync('/opt/soft/v2x/configs/chan3/cfg_multiple/publish_task/tls/app.key');
      } catch (error) {
   ...
   */
  async saveApply() {
    const { data, returnStatus } = await this.service.edge.saveApply3();
    if (!fsExtra.existsSync(`${this.app.config.algorithmicDir}chan3/task_single_list.json`)) {
      let data = [
        {
            "type": "CameraAlgoTask",
            "enable": true
      ...
      await fsExtra.writeJson(`${this.app.config.algorithmicDir}chan3/task_single_list.json`, data, { spaces: '\t' });
    }
    if (!fsExtra.existsSync(`${this.app.config.algorithmicDir}chan3/task_multiple_list.json`)) {
      let data = [
        {
      ...
      await fsExtra.writeJson(`${this.app.config.algorithmicDir}chan3/task_multiple_list.json`, data, { spaces: '\t' });
    }
