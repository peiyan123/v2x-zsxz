...
class RadarController extends BaseController {
  async get() {
    const { ctx } = this;
    const result = await this.service.radar.getAll();
    this.success(result);
  }
  async create() {
    const { ctx } = this;
    ...
    if (!valid) return;
    const result = await this.service.radar.create(requestData);
    if (result.returnStatus === 'SUCCEED') {
      const groupName = await this.service.group.getGroupNameById(requestData.groupId);
      // 新建雷达时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail({ radarType: requestData.model, operatorType: 'LIDAR', radarIp: requestData.ip, deviceSn: requestData.sn, id: result.data.id, groupName, ...pick(requestData, [ 'port', 'protocol', 'mbcIp', 'mbcPort', 'mbcNetworkCard' ]) });
      this.success();
    } else {
  ...
  async update() {
    ...
    if (!valid) return;
    const result = await this.service.radar.update(requestData);
    if (result.returnStatus === 'SUCCEED') {
      const groupName = await this.service.group.getGroupNameById(requestData.groupId);
      // 更新雷达时需调用EdgeService的保存通道算法配置接口
      this.service.edge.saveDetail({ radarType: requestData.model, operatorType: 'LIDAR', radarIp: requestData.ip, deviceSn: requestData.sn, groupName, ...pick(requestData, [ 'id', 'port', 'protocol', 'mbcIp', 'mbcPort', 'mbcNetworkCard' ]) });
      this.success();
    } else {
      this.failed(result.errorMessage);
    }
  ...
  async delete() {
    ...
    if (!valid) return;
    const isExist = await this.app.model.Radar.findOne({ where: { id: radarId } });
    if (!isExist) return this.failed('ID不存在');
    // const dir = positionName[String(isExist.position)];
    const dir = await this.service.group.getGroupNameById(isExist.groupId);
    const result = await this.service.radar.delete(radarId);
  ...
  async writeDeviceData() {
    ...
    const emptyArr = [];
    const allDeviceData = await this.service.radar.getAll();
  ...
  async reportData() {
    ...
    for (let i = 0; i < allDeviceData.length; i++) {
      const groupName = await this.service.group.getGroupNameById(allDeviceData[i].groupId);
  ...
  async writeRreportData(data) {
    const isDirExsit = await fsExtra.pathExists(this.app.config.algorithmicDir + 'chan1/cfg_single/' + data.positionName);
    if (!isDirExsit) return false;
    const fileName = this.app.config.algorithmicDir + 'chan1/cfg_single/' + data.positionName + '/sensor.json';
  ...
  async deleteRadarConfig(dir) {
    const fileName = this.app.config.algorithmicDir + 'chan1/cfg_single/' + dir + '/sensor.json';
    const isFileExsit = await fsExtra.pathExists(fileName);
    if (isFileExsit) {
      const fileData = await fsExtra.readJson(fileName);
