...
class TaskController extends BaseController {
  ...
  async saveSingleConfig() {
    ...
    await this.service.task.writeFile(requestData, 'single');
    this.success();
  }
  /**
  ...
  async saveMultipleConfig() {
    ...
    await this.service.task.writeFile(requestData, 'multiple');
    // // 将配置保存在.config目录中，以便恢复
    // this.app.locals.store.multipleTask.set({ data: requestData });
    // 将配置保存在数据库中，以便恢复
    this.service.mec.saveMecConfig({ id: 1, multipleTask: requestData });
    this.success();
  }
   ...
   */
  async getTaskConfig() {
    const result = await this.service.task.getAllData();
    this.success(result);
  }
