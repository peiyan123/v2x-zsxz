...
class CameraService extends Service {
  async getAll() {
    const result = await this.app.model.Camera3.findAll({ order: [[ 'createdAt', 'DESC' ]] });
    return result;
  }
  /**
  ...
  async getByNameAndRtsp(searchParams, orderInfo, pageParams) {
    ...
    const { rows, count } = await this.app.model.Camera3.findAndCountAll({
      where: whereClause,
      order: orderInfo,
      ...pageInfo,
  ...
  async create(data) {
    ...
    const result = await this.app.model.Camera3.create(data);
    return { data: result, returnStatus: 'SUCCEED' };
  }
  async update(data) {
    const isExist = await this.app.model.Camera3.findOne({ where: { id: data.id } });
    ...
    const result = await this.app.model.Camera3.update(data, {
      where: { id: data.id },
    });
    return { data: result, returnStatus: 'SUCCEED' };
  }
  ...
  async upsertCamera(data) {
    ...
    const cameraInfo = await this.app.model.Camera3.findOne({ where: { groupId: data.groupId } });
    let cameraId = null;
    // 未找到，则添加
    if (!cameraInfo) {
  ...
  async delete(id) {
    ...
    const result = await this.app.model.transaction(t => {
      return this.app.model.Camera3.destroy({
        where: { id },
        transaction: t,
      }).then(() => {
        return this.app.model.Mark3.destroy({
          where: { camera_id: id },
          transaction: t,
        });
      });
  ...
  async getGroupId(id) {
    return await this.app.model.Camera3.findOne({ where: { id }, attributes: [ 'groupId' ] });
  }
}
