...
class MarkService extends Service {
  ...
  async create(cameraId, data, imageInfo) {
    ...
    const result = await this.app.model.transaction(async () => {
      for (let i = 0; i < insertData.length; i++) {
        await this.app.model.Mark3.upsert(insertData[i]);
      }
      // 执行数据库删除操作
      for (let i = 0; i < dataToBeDeleted.length; i++) {
        await this.app.model.Mark3.destroy({ where: dataToBeDeleted[i] });
      }
    });
    return result;
  }

  async handleMarkData(params,cameraId) {
  ...
  async getMarkData(cameraId) {
    const result = await this.app.model.Mark3.findAll({
  ...
  async deleteData(cameraId) {
    const result = await this.app.model.Mark3.destroy({ where: { camera_id: cameraId } });
    return result;
  }
  ...
  async getReportData() {
    ...
    const result3 = await this.app.model.Camera3.findAll();
    const cameraData = this.ctx.helper.groupBy(result3, item => item.id);
