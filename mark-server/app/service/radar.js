...
class RadarService extends Service {
  async getAll() {
    const result = await this.app.model.Radar.findAll({
  ...
  async create(data) {
    ...
    const result = await this.app.model.Radar.create(data);
    return { data: result, returnStatus: 'SUCCEED' };
  }
  async update(data) {
    const isExist = await this.app.model.Radar.findOne({where: {id: data.id}})
    ...
    const result = await this.app.model.Radar.update(data, {
      where: { id: data.id },
    });
    return { data: result, returnStatus: 'SUCCEED' };
  }
  ...
  async upsertRadar(data) {
    ...
    const radarInfo = await this.app.model.Radar.findOne({ where: { groupId: data.groupId } });
    // 未找到，则添加
    if (!radarInfo) {
  ...
  async delete(id) {
    const result = await this.app.model.Radar.destroy({
      where: { id },
    });
    return result;
  }
  ...
  async getRadarByGroupId(groupId) {
    const result = await this.app.model.Radar.findOne({
      where: { groupId },
    });
    return result;
  }
