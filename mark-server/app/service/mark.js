...
class MarkService extends Service {
  ...
  async create(cameraId, data, imageInfo) {
    ...
    const result = await this.app.model.transaction(async () => {
      for (let i = 0; i < insertData.length; i++) {
        await this.app.model.Mark.upsert(insertData[i]);
      }
      // 执行数据库删除操作
      for (let i = 0; i < dataToBeDeleted.length; i++) {
        await this.app.model.Mark.destroy({ where: dataToBeDeleted[i] });
      }
    });
    return result;
  }
  ...
  async getMarkData(cameraId) {
    const result = await this.app.model.Mark.findAll({
  ...
  async deleteData(cameraId) {
    const result = await this.app.model.Mark.destroy({ where: { camera_id: cameraId } });
    return result;
  }
  ...
  async getReportData() {
    ...
    const result2 = await this.app.model.Camera.findAll();
