...
class GroupService extends Service {
   ...
   */
  async getAll() {
    const result = await this.app.model.Group2.findAll();
    return result;
  }
  /**
   * 创建组
  ...
  async create(data) {
    ...
    const result = await this.app.model.Group2.create(data);
   ...
   */
  async update(data) {
    const isExist = await this.app.model.Group2.findOne({ where: { id: data.id } });
    ...
    const result = await this.app.model.Group2.update(data, {
      where: { id: data.id },
    });
    return { data: result, returnStatus: 'SUCCEED' };
  }
   ...
   */
  async delete(id) {
    const group = await this.app.model.Group2.findOne({
      where: { id },
      // 获取关联的摄像头和雷达id
      include: [{
        model: this.app.model.Camera2,
        attributes: [ 'id' ],
      }, {
        model: this.app.model.Radar2,
        attributes: [ 'id' ],
      }],
    });
    ...
    try {
      ...
      const deleteIds = [];
      if (group.camera2) {
        deleteIds.push({ operatorType: 'VIDEO', ids: [ group.camera2.id ] });
      }
      if (group.radar2) {
        deleteIds.push({ operatorType: 'LIDAR', ids: [ group.radar2.id ] });
      }
  ...
  async getGroupInfo(whereClause) {
    const result = await this.app.model.Group2.findOne({
      where: whereClause,
      include: [{
        model: this.app.model.Camera2,
        attributes: { include: transformTimezone('camera2') },
      }, {
        model: this.app.model.Radar2,
        attributes: { include: transformTimezone('radar2') },
      }],
    })
    return { data: result, returnStatus: 'SUCCEED' };
  }
  ...
  async getGroupBasicInfoById(id) {
    const result = await this.app.model.Group2.findOne({ where: { id } });
    return result;
  }
  /**
   * 获取组的名字
  ...
  async deleteAllGroups() {
    const deleteIds = [];
    const cameraIds = await this.app.model.Camera2.findAll({
      attributes: [ 'id' ],
    });
    const radarIds = await this.app.model.Radar2.findAll({
      attributes: [ 'id' ],
    });
    if (cameraIds) {
    ...
    const group = await this.app.model.Group2.findAll();
  ...
  async createGroupAndCamera(cameraIds, group) {
    if (cameraIds[group] === undefined) {
      ...
      const { data: cameraResult } = await this.service.camera2.create({ model: `${group}_1`, groupId: groupResult.id });
  ...
  async createRadar(data, group) {
    ...
    if (radarData) {
      const groupResult = await this.app.model.Group2.findOne({ where: { name: group }, attributes: [ 'id' ] });
      if (groupResult) {
  ...
  async saveConfigFiles(stream, cameraIds) {
    ...
    const reg = /^([^/]+\/)*chan2\/cfg_single\/([^/]+)\/([^/]+)\/(.+)$/;
    const matchResult = fieldname.match(reg);
    console.log(matchResult, '2222222222222222222222222')
    if (matchResult) {
      ...
      targetDir = `${app.config.algorithmicDir}chan2/cfg_single/${group}/${dir}`;
      if (otherDir) {
      ...
      const multipleResult = fieldname.match(/^([^/]+\/)*chan2\/cfg_multiple\/(.+)$/);
      if (multipleResult) {
        const otherResult = multipleResult[2].match(/(.+\/)?[^/]+$/);
        const otherDir = otherResult ? otherResult[1] || '' : '';
        targetDir = app.config.algorithmicDir + 'chan2/cfg_multiple/';
        if (otherDir) {
          targetDir = `${targetDir}/${otherDir}`;
        }
        targetPath = path.join(app.config.algorithmicDir + 'chan2/cfg_multiple/', multipleResult[2]);
          ...
          if (otherResult) {
            ...
            if (filename === 'device_info.json') {
              ...
              if (allData) {
                ...
                // this.service.edge.saveDeviceDetail2(otherData);
              }
            }
          }
        }
