...
class GroupService extends Service {
   ...
   */
  async getAll() {
    const result = await this.app.model.Group.findAll();
    return result;
  }
  /**
   * 创建组
  ...
  async create(data) {
    ...
    const result = await this.app.model.Group.create(data);
   ...
   */
  async update(data) {
    const isExist = await this.app.model.Group.findOne({ where: { id: data.id } });
    ...
    const result = await this.app.model.Group.update(data, {
      where: { id: data.id },
    });
    return { data: result, returnStatus: 'SUCCEED' };
  }
   ...
   */
  async delete(id) {
    const group = await this.app.model.Group.findOne({
      where: { id },
      // 获取关联的摄像头和雷达id
      include: [{
        model: this.app.model.Camera,
        attributes: [ 'id' ],
      }, {
        model: this.app.model.Radar,
        attributes: [ 'id' ],
      }],
    });
    ...
    try {
      ...
      const deleteIds = [];
      if (group.camera) {
        deleteIds.push({ operatorType: 'VIDEO', ids: [ group.camera.id ] });
      }
      if (group.radar) {
        deleteIds.push({ operatorType: 'LIDAR', ids: [ group.radar.id ] });
      }
  ...
  async getGroupInfo(whereClause) {
    const result = await this.app.model.Group.findOne({
      where: whereClause,
      include: [{
        model: this.app.model.Camera,
        attributes: { include: transformTimezone('camera') },
      }, {
        model: this.app.model.Radar,
        attributes: { include: transformTimezone('radar') },
      }],
    });
    return { data: result, returnStatus: 'SUCCEED' };
  }
  ...
  async getGroupBasicInfoById(id) {
    const result = await this.app.model.Group.findOne({ where: { id } });
    return result;
  }
  /**
   * 获取组的名字
  ...
  async deleteAllGroups() {
    const deleteIds = [];
    const cameraIds = await this.app.model.Camera.findAll({
      attributes: [ 'id' ],
    });
    const radarIds = await this.app.model.Radar.findAll({
      attributes: [ 'id' ],
    });
    if (cameraIds) {
    ...
    const group = await this.app.model.Group.findAll();
  ...
  async createGroupAndCamera(cameraIds, group) {
    if (cameraIds[group] === undefined) {
      ...
      const { data: cameraResult } = await this.service.camera.create({ model: `${group}_1`, groupId: groupResult.id });
  ...
  async createRadar(data, group) {
    ...
    if (radarData) {
      const groupResult = await this.app.model.Group.findOne({ where: { name: group }, attributes: [ 'id' ] });
      if (groupResult) {
  ...
  async saveConfigFiles(stream, cameraIds) {
    ...
    if (matchResult) {
      ...
      targetDir = `${app.config.algorithmicDir}chan1/cfg_single/${group}/${dir}`;
      if (otherDir) {
        ...
        if (allData) {
          ...
          if (filename === 'sensor.json') {
            ...
            if (markValue) {
              ...
              await this.service.mark.create(cameraIds[group], [ markValue ], { width: 1920, height: 1080, url: '' });
            }
          }
        }
      }
      ...
      const multipleResult = fieldname.match(/^([^/]+\/)*chan1\/cfg_multiple\/(.+)$/);
      if (multipleResult) {
        const otherResult = multipleResult[2].match(/(.+\/)?[^/]+$/);
        const otherDir = otherResult ? otherResult[1] || '' : '';
        targetDir = app.config.algorithmicDir + 'chan1/cfg_multiple/';
        if (otherDir) {
          targetDir = `${targetDir}/${otherDir}`;
        }
        targetPath = path.join(app.config.algorithmicDir + 'chan1/cfg_multiple/', multipleResult[2]);
        if (multipleResult[2] === 'fuse_multiple.json') {
          ...
          if (allData) {
            ...
            Object.keys(markValue).forEach(async group => {
              ...
              // 将解析后的数据写入到数据库的mark表中
              await this.service.mark.create(cameraIds[group], [ data ], { width: 1920, height: 1080, url: '' });
            });
          }
        }
      } else {
          ...
          if (otherResult) {
            ...
            if (filename === 'device_info.json') {
              ...
              if (allData) {
                ...
                await this.service.mec.saveMecConfig(configData);
                const { id, ...otherData } = configData;
                // 调用EdgeService保存设备信息配置的接口
                // this.service.edge.saveDeviceDetail(otherData);
              }
            }
          }
        }
