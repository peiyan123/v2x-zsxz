...
class TaskService extends Service {
  ...
  async changeDistributionSendTask(isEnable) {
    const singleFilePath = `${this.app.config.algorithmicDir}chan3/${TASK_SINGLE_LIST}.json`;
    let singleFileData = [];
  ...
  async writeFile(result, type) {
    ...
    await fsExtra.ensureDir(`${this.app.config.algorithmicDir}chan3/`);
    await fsExtra.writeJson(`${this.app.config.algorithmicDir}chan3/${filename}.json`, result, { spaces: '\t' });
  }
  /**
   * 获取所有task配置文件内容
   * @return {object} 文件内容
  ...
  async getTaskData(filename) {
    ...
    const filePath = `${algorithmicDir}chan3/${filename}.json`;
    let result = null;
