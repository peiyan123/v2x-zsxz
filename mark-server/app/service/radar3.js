...
class RadarService extends Service {
  async getAll() {
    const result = await this.app.model.Radar3.findAll({
  ...
  async create(data) {
    ...
    const result = await this.app.model.Radar3.create(data);
    return { data: result, returnStatus: 'SUCCEED' };
  }
  async update(data) {
    const isExist = await this.app.model.Radar3.findOne({where: {id: data.id}})
    ...
    const result = await this.app.model.Radar3.update(data, {
      where: { id: data.id },
    });
    return { data: result, returnStatus: 'SUCCEED' };
  }
  ...
  async upsertRadar(data) {
    ...
    const radarInfo = await this.app.model.Radar3.findOne({ where: { groupId: data.groupId } });
    // 未找到，则添加
    if (!radarInfo) {
  ...
  async delete(id) {
    const result = await this.app.model.Radar3.destroy({
      where: { id },
    });
    return result;
  }
  ...
  async getRadarByGroupId(groupId) {
    const result = await this.app.model.Radar3.findOne({
      where: { groupId },
    });
    return result;
  }
