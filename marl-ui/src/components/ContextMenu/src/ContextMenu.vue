...
const ItemContent: FunctionalComponent<ItemContentProps> = (props) => {
  const { item } = props
  return (
    <span style="width: 100%; " class="flex px-1 items-center" onClick={props.handler.bind(null, item)}>
      {props.showIcon && item.icon && <Icon class="" icon={item.icon} />}
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getStyle = computed((): CSSProperties => {
      ...
      const { x, y } = (axis as any) || { x: 0, y: 0 }
      const menuHeight = ((items as []) || []).length * 40
      const menuWidth = width as number
    ...
    function renderMenuItem(items: ContextMenuItem[]) {
      return items.map((item) => {
        const { ifShow = true, disabled, label, children, divider = false } = item
...
.ant-menu-root.ant-menu-vertical.context-menu {
  box-shadow: 0 2px 2px 0 rgba(0, 0, 0, 0.14), 0 3px 1px -2px rgba(0, 0, 0, 0.1), 0 1px 5px 0 rgba(0, 0, 0, 0.06);
}
</style>
