...
/**
 * @description: 根据 checked 筛选所需 checkedKeys
 * @param treeData TreeData
 */
export const filterCheckedKeysByTree = (treeData) => {
  const checkedKeys: (string | number)[] = []
  treeToList(treeData).map((item) => {
    if (item.checked) {
...
/**
 * @description: 获取拥有子节点的父节点
 * @param treeData TreeData
 */
export const getHasChildrenNode = (treeData) => {
  let parentNode: (string | number)[] = []
  treeData.forEach((item) => {
    if (item?.children && item?.children.length > 0) {
      // 递归遍历子节点，获取拥有子节点的节点列表，并和当前节点一起加入到结果列表中
      parentNode = [...getHasChildrenNode(item.children), ...parentNode, item.id]
    }
  })
  return parentNode
}
