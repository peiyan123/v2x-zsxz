...
import { LineArrowDecorators } from './line-arrow-decorators'
...
export class SegmentationScene2 {
  // 画布对象
  ...
  constructor(el: HTMLElement, url?: string, markData: ShapeSegmentationData[] = []) {
    ...
    this.destroyEvent.push(
      this.markCanvas.shapeSelectedAfterEvent?.subscribe((shape) => {
        ...
        // console.log(shape);
        if (!shape.shapeId.includes('shapeArrow')){
  ...
  deleteSelected() {
    ...
    if (
      ...
      // 删除箭头和mark TODO
     this.markCanvas.arrowMap?.delete(shape.shapeId!)
     this.markCanvas.shapeHoverPlan.splice(this.markCanvas.shapeHoverPlan?.findIndex((item:any) => item?.shapeId === shape.shapeId), 1)
     // this.markCanvas.shapeSelectPlan.splice(this.markCanvas.shapeSelectPlan?.findIndex((item:any) => item?.shapeId === shape.shapeId), 1)
     // 删除箭头 TODO
  ...
  drawArrow() {
    ...
    this.markModeSave()
    this.closeMarkMode()
    this.markCanvas.startDrawArrow()
  }
  // 绘制区域
  drawArea(config) {
    this.openMarkMode(config)
    this.markCanvas.stopDrawArrow()
  }
}

export default SegmentationScene2
