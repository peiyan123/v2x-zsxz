...
export function ShapeDetectionLineDecorators <T extends new (...args: any[]) => AiMarkInterface>(constructor: T) {
  return class extends constructor {
    ...
    drawShapeDetectionLine() {
      this.canvasOperation(() => {
        ...
        this.shapeDetections?.forEach(shape => this.updateShapeMinMax!(shape, this.shapePeripheralDistance!))
        ...
        detectionsLine = (shape: ShapeDetectionData) => {
          ...
          // draw lines
          lines.push({ start: shape.point![0], end: shape.point![1], config: {color: shape.color}})
          lines.push({ start: shape.point![1], end: shape.point![2], config: {color: shape.color}})
          lines.push({ start: shape.point![2], end: shape.point![3], config: {color: shape.color}})
          lines.push({ start: shape.point![3], end: shape.point![0], config: {color: shape.color}})
          if (!shape.label) return
          // label font
          fonts.push({ x: shape.minX, y: shape.maxY,
            label: shape.label, labelColor: shape.labelColor,
