    ...
    pointHoverModel() {
      this.destroy.push(
        this.canvasElementEvents!.mousemove.subscribe((e) => {
          ...
          if (this.pointSelectedPlanMap)
            this.pointSelectedPlanMap.forEach((typePointArrary) => points.push(...typePointArrary))
          // 可以被移动的点，可以悬浮高亮
          ...
          if (points.length) {
            // 符合距离的点
            const hoverList = points?.filter(
              (p: Point) =>
                Point2Distance({ x: p.x!, y: p.y! }, { x: e.offsetX, y: e.offsetY }) < this.pointHoverDistanceRange
            )
            if (hoverList.length) {
      ...
      this.destroy.push(
        this.canvasElementEvents!.touchmove.subscribe((e) => {
          ...
          if (points.length) {
            ...
            const hoverList = points?.filter(
              (p: Point) =>
                Point2Distance({ x: p.x!, y: p.y! }, { x: offsetX, y: offsetY }) < this.pointHoverDistanceRange
            )
            if (hoverList.length) {
              // 如果 当前 hover 的点 在 备选的hover列表里，则保持当前点的选中状态
              if (hoverList.includes(this.pointHover)) return
