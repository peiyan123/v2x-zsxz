...
import { RatioCalculation, getVertexPosition } from '../util/shape-calculation'
    ...
    pointsMouseDrag() {
      ...
      this.destroy.push(
        this.canvasElementEvents!.mousemove.subscribe((e) => {
          ...
          if (this.pointDragDrawing && this.pointDragingmouseDown) {
            ...
            ;(this.pointDragDrawing!.source.x = RatioCalculation(
              x - this.imageStartPoint!.x,
              this.canvasZoom,
              this.accuracy
            )),
              (this.pointDragDrawing!.source.y = RatioCalculation(
      ...
      this.destroy.push(
        fromEvent(window, 'mouseup').subscribe(() => {
          if (this.pointDragDrawing) {
            ...
            // 设置isPointDragEnd为true，防止取消形状选择，在ShapeSelectDecorators类中使用
            this.isPointDragEnd = true
            // 延迟重置isPointDragEnd为false，延迟的时间是经验值，太短不起作用，太长可能影响下一次点的拖动
            setTimeout(() => {
              this.isPointDragEnd = false
            }, 400)
          }
        })
      )
      ...
      this.destroy.push(
        this.canvasElementEvents!.touchstart.subscribe((e) => {
          ...
          if (!this.pointDragDrawing && this.pointHover && pointsMouse.includes(this.pointHover)) {
            ...
            this.pointDragingmouseDown = { x: e.touches[0].screenX, y: e.touches[0].screenY }
            this.pointDragDrawing = this.pointHover
            this.pointDragingmouseDownPoint = { x: this.pointHover.x, y: this.pointHover.y }
            this.pointDragStartEvent?.next(this.pointDragDrawing!)
            this.pointDragEvent.next(this.pointDragDrawing!)
      ...
      this.destroy.push(
        this.canvasElementEvents!.touchmove.subscribe((e) => {
          const vertex = getVertexPosition(e.target)
          const offsetX = Math.round(e.touches[0].pageX - vertex.left)
          const offsetY = Math.round(e.touches[0].pageY - vertex.top)
          // pointDragDrawing 当前移动的点，pointDragingmouseDown 鼠标左键按下的位置
          if (this.pointDragDrawing && this.pointDragingmouseDown) {
            // 点在图片上的判断
            if (!this.imageRangeOnXY!(offsetX, offsetY)) return
            // 新点的位置
            const x = this.pointDragingmouseDownPoint!.x! + (e.touches[0].screenX - this.pointDragingmouseDown.x)
            const y = this.pointDragingmouseDownPoint!.y! + (e.touches[0].screenY - this.pointDragingmouseDown.y)
            ;(this.pointDragDrawing!.source.x = RatioCalculation(
              x - this.imageStartPoint!.x,
              this.canvasZoom,
              this.accuracy
      ...
      this.destroy.push(
        fromEvent(window, 'touchend').subscribe(() => {
          if (this.pointDragDrawing) {
            this.pointDragEvent.next(this.pointDragDrawing!)
            this.pointDragEndEvent?.next(this.pointDragDrawing!)
            this.pointDragDrawing = null
