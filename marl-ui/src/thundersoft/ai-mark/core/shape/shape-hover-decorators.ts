import { AiMarkInterface, RatioConversion } from '..'
  ...
  return class extends constructor {
    ...
    drawHoverShape() {
      this.canvasOperation((ctx: CanvasRenderingContext2D) => {
        ...
        if (this.shapeHover) {
          // if (this.shapeHoverBackground) {
          ctx.beginPath()
          ctx.fillStyle = `rgba( ${this.shapeHover.color!.split('(')[1].split(')')[0]} ,.1)` // 区域
          ...
          // } else {
          ctx.beginPath()
          ctx.fillStyle = 'rgba(162,214,234, 0)' // 区域
          ctx.strokeStyle = `rgba( ${this.shapeHover.color!.split('(')[1].split(')')[0]} ,.8)` // 框
          ctx.setLineDash([0])
      ...
      this.destroy.push(
        this.canvasElementEvents!.mousemove.subscribe((e) => {
          ...
          const hoverList: { shape: Shape; dist: number }[] = [],
            shapeHoverPlan: Shape[] = [...this.shapeSelectPlan!, ...this.shapeHoverPlan!]
     ...
     * 计算 图形的 最小 外接距
     */
    //  RatioConversion(line.start.source.x, this.canvasZoom, this.accuracy) + this.imageStartPoint!.x TODO lgj
    updateShapeMinMax(shape: Shape, shapePeripheralDistance: number) {
      if (shape.point && shape.point.length) {
        shape.minX = Math.min(...(shape.point!.map((p) => RatioConversion(p.source.x, this.canvasZoom, this.accuracy) + this.imageStartPoint!.x) as number[])) - shapePeripheralDistance
        shape.maxX = Math.max(...(shape.point!.map((p) => RatioConversion(p.source.x, this.canvasZoom, this.accuracy) + this.imageStartPoint!.x) as number[])) + shapePeripheralDistance
        shape.minY = Math.min(...(shape.point!.map((p) => RatioConversion(p.source.y, this.canvasZoom, this.accuracy) + this.imageStartPoint!.y) as number[])) - shapePeripheralDistance
        shape.maxY = Math.max(...(shape.point!.map((p) => RatioConversion(p.source.y, this.canvasZoom, this.accuracy) + this.imageStartPoint!.y) as number[])) + shapePeripheralDistance
      } else {
        shape.minX = 0
        shape.maxX = 0
        shape.minY = 0
