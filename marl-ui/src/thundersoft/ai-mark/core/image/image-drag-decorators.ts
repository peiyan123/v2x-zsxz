...
import { Point2Distance, getVertexPosition } from '../util'
    ...
    imageDrag() {
      ...
      this.destroy.push(
        this.canvasElementEvents!.touchstart.subscribe((e) => {
          ...
          if (this.pointHover && Point2Distance({ x: this.pointHover.x!, y: this.pointHover.y! }, { x: offsetX, y: offsetY }) < this.pointHoverDistanceRange) return
          e.returnvalue = false
          // e.preventDefault()
          
          this.imageDragingStartPoint = { x: e.touches[0].screenX, y: e.touches[0].screenY }
          this.imageDrawStartImagePosition = { x: this.imageStartPoint!.x, y: this.imageDrawStartImagePosition.y = this.imageStartPoint!.y }
          return false
        })
      )
      ...
      this.destroy.push(
        this.canvasElementEvents!.touchmove.subscribe((e) => {
          ...
          if (this.imageDragingStartPoint) {
            const x = this.imageDrawStartImagePosition.x + (e.touches[0].screenX - this.imageDragingStartPoint.x)
            const y = this.imageDrawStartImagePosition.y + (e.touches[0].screenY - this.imageDragingStartPoint.y)
            // // 限制图片不能拖动出画布范围
            // if (Math.abs(x) > (this.canvasElement.width-60) || Math.abs(y) > (this.canvasElement.height-60) ) {
            //   return
            // }
