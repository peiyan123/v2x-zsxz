...
export function ImageLoadDecorators<T extends new (...args: any[]) => AiMarkInterface>(constructor: T) {
  return class extends constructor {
    ...
    loadImageUrl(url: string): Promise<HTMLImageElement> {
      ...
      return new Promise((resolve) => {
        this.image.onload = () => {
          this.imageAspectRatio = RatioCalculation(this.image.width, this.image.height, this.accuracy)
          resolve(this.image)
        }
      })
    }
    ...
    canvasZoomDefault() {
      ...
      if (this.imageAspectRatio > this.canvasAspectRatio) {
        // 缩放 通过图片宽度缩放进行界面适配
        this.canvasZoom = RatioCalculation(this.canvasElement.width, this.image.width, this.accuracy)
        // 图片居中
        this.imageStartPoint.y =
          (this.canvasElement.height - RatioConversion(this.image.height, this.canvasZoom, this.accuracy)) / 2
        this.imageStartPoint.x = 0
      } else {
        // 缩放 通过图片高度缩放进行界面适配
        this.canvasZoom = RatioCalculation(this.canvasElement.height, this.image.height, this.accuracy)
        // 图片居中
        this.imageStartPoint.x =
          (this.canvasElement.width - RatioConversion(this.image.width, this.canvasZoom, this.accuracy)) / 2
